...
def _load_source_sites(professor: str) -> list[str]:
    ...
    meta = Path(f"07_visualization/step_results/{professor}_source_sites.json")
    if meta.exists():
        try:
...
def _wordcloud_image_path(professor: str) -> Path:
    return Path(f"05_domain_analysis/step_results/{professor}_step05_wordcloud.png")
...
def _load_prof_lexicon() -> tuple[dict[str, str], dict[str, str]]:
    p = Path("05_domain_analysis/config/professional_tag_lexicon.yaml")
    if not p.exists():
        return {}, {}
    try:
...
def _load_paper_catalog(
    ...
    step03 = Path(f"03_author_disambiguation/step_results/{professor}_step03_disambiguated_papers.jsonl")
    step05 = Path(f"05_domain_analysis/step_results/{professor}_step05_paper_domains.jsonl")
    step04 = Path(f"04_keyword_entity/step_results/{professor}_step04_keyword_entity.jsonl")
    step02 = Path(f"02_paper_list_extend/step_results/{professor}_expanded_papers.jsonl")
...
def _run_tag_learning_and_refresh(prefix: str) -> tuple[bool, str]:
    ...
    step04 = project_root / f"04_keyword_entity/step_results/{prefix}_step04_keyword_entity.jsonl"
    step06_paper = project_root / f"05_domain_analysis/step_results/{prefix}_step05_paper_domains.jsonl"
    step06_domains = project_root / f"05_domain_analysis/step_results/{prefix}_step05_domains.json"
    wordcloud_terms = project_root / f"05_domain_analysis/step_results/{prefix}_step05_wordcloud_terms.json"
    wordcloud_image = project_root / f"05_domain_analysis/step_results/{prefix}_step05_wordcloud.png"
    timeline_out = project_root / f"06_temporal_analysis/step_results/{prefix}_step06_project_timeline.json"
    allocation_out = project_root / f"06_temporal_analysis/step_results/{prefix}_step06_effort_allocation.json"
    report_out = project_root / f"06_temporal_analysis/step_results/{prefix}_step06_trend_report.md"
    duckdb_path = project_root / "07_visualization/step_results/pipeline.duckdb"

    cmds: list[list[str]] = [
        [
            str(python),
            "05_domain_analysis/processing/tag_taxonomy/review_queue_3layer.py",
            "--purge-learned",
            "--min-score",
            "8",
            "--min-len-ascii",
        ...
        [
            str(python),
            "05_domain_analysis/processing/tag_taxonomy/build_tag_report.py",
            "--queue-l2",
            "05_domain_analysis/step_results/learning_queue_l2.jsonl",
            "--queue-l3",
            "05_domain_analysis/step_results/learning_queue_l3.jsonl",
            "--learned-l2",
            "05_domain_analysis/step_results/learned_l2.jsonl",
            "--learned-l3",
            "05_domain_analysis/step_results/learned_l3.jsonl",
            "--output",
            "05_domain_analysis/step_results/tag_report.json",
        ],
        [
            str(python),
            "05_domain_analysis/processing/run_step05_domain_analysis.py",
            "--input",
            str(step04),
            ...
            "--domains-output",
            str(step06_domains),
            "--wordcloud-terms-output",
            str(wordcloud_terms),
            "--wordcloud-image-output",
            str(wordcloud_image),
            "--model",
        ...
        [
            str(python),
            "06_temporal_analysis/processing/run_step06_temporal_analysis.py",
            "--input",
            str(step06_paper),
            "--timeline-output",
            str(timeline_out),
        ...
        [
            str(python),
            "06_temporal_analysis/processing/build_step06_duckdb.py",
            "--professor",
            prefix,
            "--paper-domains",
            str(step06_paper),
...
st.set_page_config(page_title="教授研究分析可视化", layout="wide")

st.title("教授研究分析可视化")
st.caption("基于连续步骤 01-08 的本地 DuckDB + Streamlit + Plotly 展示页，可按教授、时间范围和标签层级筛选。")

db_path = st.sidebar.text_input("DuckDB 文件路径", value="07_visualization/step_results/pipeline.duckdb")
...
st.subheader("0) 一键搜索与重跑")
with st.expander("输入作者与种子论文，执行全流程（01->08）", expanded=False):
    c1, c2 = st.columns(2)
    with c1:
...
st.subheader("6) 研究词云")
with st.container():
    wc_path = _wordcloud_image_path(professor)
    if wc_path.exists():
        st.caption("来源：Step05 领域分析中的标签权重聚合结果，用于展示当前教授在研究主题上的高频重点。")
...
with st.expander("展开执行：learned 去重清洗 → 重新计算 L1/L2/L3 → 刷新 DuckDB/图表/论文清单", expanded=False):
    st.markdown(
        ...
        "- 然后重跑 `Step05` 生成新的 L1/L2/L3 分类与词云结果；\n"
        "- 再重跑 `Step06` 时间分析与 `Step07` DuckDB 写入，使图表与论文清单同步更新。\n"
    )
...
st.caption(
    "说明：当前页面展示新的 01-08 连续步骤结构结果。时间轴按年度节点统计，起始于 2021-01-01；预测默认展示未来 2 个年度节点。"
)
