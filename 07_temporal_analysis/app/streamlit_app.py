...
def _is_first_author(authors: list[str], target_author: str) -> str:
    ...
    if not target_author.strip():
        return "未设置目标作者"
    first = _norm_name(str(authors[0]))
    target = _norm_name(target_author)
    return "是" if target and (target in first or first in target) else "否"
...
def _load_paper_catalog(
    ...
    # Prefer Step04 full-feature rows (keywords, authors, venue).
...
def _predict_with_model(
    ...
    for domain in sorted(df_time["domain_name"].dropna().unique()):
        ...
        for i, pred in enumerate(fut_slice):
            pred_rows.append(
                {
                    "month": first_ts + pd.DateOffset(months=3 * i),
...
st.subheader("2) 领域时间折线（半年论文发布频率，2021起）")
if df_time.empty:
    st.info("该筛选范围内无数据。")
else:
    hist = _fill_history_quarters(df_time, "paper_count", min_start="2021-01-01", months_step=6)
    ...
    tick_vals, tick_texts = _halfyear_tick_labels(q_start, q_end)
    fig.update_xaxes(
        tickmode="array",
        tickvals=tick_vals,
        ticktext=tick_texts,
        range=[q_start, q_end + pd.DateOffset(months=6)],
    )
    fig.update_yaxes(rangemode="tozero", range=[0.0, max(1.0, max_cnt * 1.15)])
    st.plotly_chart(fig, use_container_width=True)
    if not model_meta_cnt.empty:
        st.caption("论文频率预测模型与误差（回测）")
        st.dataframe(model_meta_cnt, use_container_width=True)

st.subheader("3) 领域时间折线（半年精力/质量加权，2021起）")
if df_time.empty:
    st.info("该筛选范围内无数据。")
else:
    hist_w = _fill_history_quarters(df_time, "weight", min_start="2021-01-01", months_step=6)
    ...
    if not model_meta_w.empty:
        ...
        st.dataframe(model_meta_w, use_container_width=True)
...
paper_df = _load_paper_catalog(professor, start, end, target_author)
if paper_df.empty:
...
with st.expander("展开查看全部论文（标题/期刊/时间/一作判断/关键词）", expanded=False):
    if paper_df.empty:
        st.info("暂无可展示论文。")
    else:
...
st.caption(
    "说明：时间轴按半年节点（每6个月）统计，起始于2021-01-01；预测默认展示未来2个半年节点。"
)
